...
from Base.GUIPoint import PointSzieRelated
...
class AutoScriptMaker:
    ...
    def SaveProject(self)->bool:
        if not hasattr(self, '_AutoScriptMaker__scriptpath'):            
            return False
        BMins.save()
        SDMins.save()
        return True
    def DelProject(self)->bool:
        return True
    def Get_GUIPoint(self,PointType:PointType=PointType.Center,PointSzieRelated:PointSzieRelated=PointSzieRelated.WindowHigh)->GUIPoint:
        #TODO:通过双击屏幕计算GUIPoint
        (x,y) = KMMins.Get_mouse_when_double_click()
        point = Vector2(x,y)
        resGP = GUIPoint(point,type=PointType,related_size=PointSzieRelated)
        return resGP
    def Add_Button(self,name,PointType:PointType,PointSzieRelated:PointSzieRelated)->bool:
        print("please double click to locate the button")
        butopoint = self.Get_GUIPoint(PointType,PointSzieRelated)
        return BMins.Add_Button(name,butopoint)
    def Add_ScreenDetector(self,name,PointType:PointType,PointSzieRelated:PointSzieRelated)->bool:
        flag = False
        print("please double click to locate the region")
        deteLT = self.Get_GUIPoint(PointType,PointSzieRelated)
        print("please double click again to locate the region")
        deteRB = self.Get_GUIPoint(PointType,PointSzieRelated)
        print(f"please double click to locate the img")
        imgLT = self.Get_GUIPoint(PointType,PointSzieRelated)
        print(f"please double click again to locate the img")
        imgRB = self.Get_GUIPoint(PointType,PointSzieRelated)
